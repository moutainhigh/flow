...
public class StringUtil {
  ...
  public static int getSubtringCount(String source,String sub) {
	...
	public static boolean isExists(String key, List<String> list){
		...
		for(int i = 0; i < list.size(); i++){
			if (key.toLowerCase().equals(list.get(i).toLowerCase())){
				return true;
			}
		}
	...
	public static String getString(Object[] path) {
		StringBuffer sb = new StringBuffer();
		for(Object str:path){
	 ...
	 * @return
	 */
	public static String clearSQLComment(String sqlText){
		 ...
		 * 
		 */
		String otherPatten = "：!\\\" \\t#$%&'()*+,-./:;<=>?@\\[\\]^_`{|}~<>";
		sqlBuffer.append(sqlText.replaceAll("--[\\S, " + otherPatten + "&&[^*/]]*[\r\n]*", "\r\n"));
		//System.out.println("【" + sqlText + "】");
		//2.清除/**/的注释
		int index = sqlBuffer.indexOf("/*");
		
		while (index > -1){
			//System.out.println(sqlBuffer.length() + ": "  + index + " ,"+  sqlBuffer.indexOf("*/") + 2);
			//System.out.println("sqlBuffer = " + sqlBuffer);
			sqlBuffer.delete(index == 0 ? 0 : index -1, sqlBuffer.indexOf("*/") + 2);

			//test = test.substring(0, index == 0 ? 0 : index -1) + test.substring(test.indexOf("*/") + 3);//test.indexOf("*/") + 3:一定要加3哦.
			index = sqlBuffer.indexOf("/*");
		}
