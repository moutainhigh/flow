 ...
 * 大纲列表选择
 */
public class OutlineChooser extends JDialog {
    ...
    public static void main(String[] args) {
		...
		OutlineChooser dialog = new OutlineChooser(null,flowMetaData,"admin",false);
		dialog.showDialog();
    }

    public OutlineChooser(JFrame parent, FlowMetaData fmd, String userID,
	    boolean running) {
	super(parent, "大纲选择(将该流程导入到哪个大纲中？)", true);
	...
	lstFlows.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);   
    ...
    private class ButtonHandler implements ActionListener {
    	public void actionPerformed(ActionEvent arg0) {
    		...
    		if (btn == btnOk) {
    			isApproved = true;
    			OutlineChooser.this.setVisible(false);
    		} else if (btn == btnCancel) {
    			isApproved = false;
    			OutlineChooser.this.dispose();
    		}
    	}
    }
